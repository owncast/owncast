...
func (s *Service) NodeInfoController(w http.ResponseWriter, r *http.Request) {
	type links struct {
		Rel  string `json:"rel"`
	...
	serverURL := s.Persistence.Data.GetServerURL()
	if serverURL == "" {
	...
	if err := s.writeResponse(res, w); err != nil {
		log.Errorln(err)
	}
}

// NodeInfoV2Controller returns the V2 node info response.
func (s *Service) NodeInfoV2Controller(w http.ResponseWriter, r *http.Request) {
	...
	localPostCount, _ := s.Persistence.GetLocalPostCount()
	...
	res := response{
		...
		Metadata: metadata{
			ChatEnabled: !s.Persistence.Data.GetChatDisabled(),
		},
	}
...
func (s *Service) XNodeInfo2Controller(w http.ResponseWriter, r *http.Request) {
	...
	res := &response{
		Organization: Organization{
			Name:    s.Persistence.Data.GetServerName(),
			Contact: serverURL,
		},
		Server: Server{
			BaseURL:  serverURL,
...
func (s *Service) InstanceV1Controller(w http.ResponseWriter, r *http.Request) {
	type Stats struct {
	...
	res := response{
		URI:              serverURL,
		Title:            s.Persistence.Data.GetServerName(),
		ShortDescription: s.Persistence.Data.GetServerSummary(),
		Description:      s.Persistence.Data.GetServerSummary(),
...
func (s *Service) writeResponse(payload interface{}, w http.ResponseWriter) error {
	...
	return follower.WritePayloadResponse(payload, w, s.Crypto, publicKey)
}

// HostMetaController points to webfinger.
func (s *Service) HostMetaController(w http.ResponseWriter, r *http.Request) {
	serverURL := s.Persistence.Data.GetServerURL()
	if serverURL == "" {
		w.WriteHeader(http.StatusNotFound)
		return
