...
func (s *Service) MakeActorFromExernalAPEntity(entity ExternalEntity) (*ActivityPubActor, error) {
	...
	username := s.GetFullUsernameFromExternalEntity(entity)
...
func (s *Service) MakeActorPropertyWithID(idIRI *url.URL) vocab.ActivityStreamsActorProperty {
...
func (s *Service) MakeServiceForAccount(accountName string) vocab.ActivityStreamsService {
	actorIRI := s.MakeLocalIRIForAccount(accountName)

	person := streams.NewActivityStreamsService()
	nameProperty := streams.NewActivityStreamsNameProperty()
	nameProperty.AppendXMLSchemaString(s.data.GetServerName())
	...
	inboxIRI := s.MakeLocalIRIForResource("/user/" + accountName + "/inbox")
	...
	needsFollowApprovalProperty.Set(s.data.GetFederationIsPrivate())
	person.SetActivityStreamsManuallyApprovesFollowers(needsFollowApprovalProperty)

	outboxIRI := s.MakeLocalIRIForResource("/user/" + accountName + "/outbox")
	...
	if t, err := s.data.GetServerInitTime(); t != nil {
	...
	// Avatar
	uniquenessString := s.data.GetLogoUniquenessString()
	userAvatarURLString := s.data.GetServerURL() + "/logo/external"
	...
	summaryProperty.AppendXMLSchemaString(s.data.GetServerSummary())
	person.SetActivityStreamsSummary(summaryProperty)

	// Links
	if serverURL := s.data.GetServerURL(); serverURL != "" {
		s.addMetadataLinkToProfile(person, "Stream", serverURL)
	}
	for _, link := range s.data.GetSocialHandles() {
		s.addMetadataLinkToProfile(person, link.Platform, link.URL)
	}
	...
	for _, tagString := range s.data.GetServerMetadataTags() {
		hashtag := s.MakeHashtag(tagString)
	...
	if len(s.data.GetSocialHandles()) == 1 {
		s.addMetadataLinkToProfile(person, "Owncast", "https://owncast.online")
	}
...
func (s *Service) GetFullUsernameFromExternalEntity(entity ExternalEntity) string {
...
func (s *Service) addMetadataLinkToProfile(profile vocab.ActivityStreamsService, name string, url string) {
