...
func (s *Service) handleEngagementActivity(eventType events.EventType, isLiveNotification bool, actorReference vocab.ActivityStreamsActorProperty, action string) error {
	...
	if !s.data.GetFederationShowEngagement() {
		return nil
	}
	...
	actor, _ := s.follower.GetResolvedActorFromActorProperty(actorReference)
	...
	if isLiveNotification && action == events.FediverseEngagementLike {
		...
		suffix = fmt.Sprintf("liked a post from %s.", s.data.GetServerName())
		...
		suffix = fmt.Sprintf("shared a post from %s.", s.data.GetServerName())
