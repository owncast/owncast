...
func (s *Service) handle(request apmodels.InboxRequest) {
	...
	if err := s.follower.Resolve(context.Background(), request.Body,
		s.handleUpdateRequest, s.handleFollowInboxRequest, s.handleLikeRequest,
		s.handleAnnounceRequest, s.handleUndoInboxRequest, s.handleCreateRequest,
	); err != nil {
...
func (s *Service) Verify(request *http.Request) (bool, error) {
	...
	publicKey, err := s.follower.GetResolvedPublicKeyFromIRI(pubKeyID.String())
	if err != nil {
...
func (s *Service) isBlockedDomain(domain string) bool {
	blockedDomains := s.data.GetBlockedFederatedDomains()
...
func (s *Service) isBlockedActor(actorIRI *url.URL) (bool, error) {
	blockedactor, err := s.follower.GetFollower(actorIRI.String())
