...
func WriteStreamResponse(item vocab.Type, w http.ResponseWriter, s *crypto.Service, publicKey crypto.PublicKey) error {
	...
	return WriteResponse(b, w, s, publicKey)
}

// WritePayloadResponse will write any arbitrary object to the provided ResponseWriter and sign with the provided key.
func WritePayloadResponse(payload interface{}, w http.ResponseWriter, s *crypto.Service, publicKey crypto.PublicKey) error {
...
func WriteResponse(payload []byte, w http.ResponseWriter, s *crypto.Service, publicKey crypto.PublicKey) error {
	w.Header().Set("Content-Type", "application/activity+json")

	if err := s.SignResponse(w, payload, publicKey); err != nil {
...
func CreateSignedRequest(payload []byte, url *url.URL, fromActorIRI *url.URL, crypto *crypto.Service) (*http.Request, error) {
