...
func (s *Service) SignResponse(w http.ResponseWriter, body []byte, publicKey PublicKey) error {
	privateKey := s.GetPrivateKey()

	return s.signResponse(privateKey, *publicKey.ID, body, w)
}

func (s *Service) signResponse(privateKey crypto.PrivateKey, pubKeyID url.URL, body []byte, w http.ResponseWriter) error {
...
func (s *Service) SignRequest(req *http.Request, body []byte, actorIRI *url.URL) error {
...
func (s *Service) signRequest(privateKey crypto.PrivateKey, pubKeyID string, body []byte, r *http.Request) error {
	prefs := []httpsig.Algorithm{httpsig.RSA_SHA256}
	digestAlgorithm := httpsig.DigestSha256
...
func (s *Service) CreateSignedRequest(payload []byte, url *url.URL, fromActorIRI *url.URL) (*http.Request, error) {
