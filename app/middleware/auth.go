...
func RequireAdminAuth(handler http.HandlerFunc, d *data.Service) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		username := "admin"
		password := d.GetAdminPassword()
...
func RequireExternalAPIAccessToken(scope string, handler ExternalAccessTokenHandlerFunc, d *data.Service) http.HandlerFunc {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		integration, err := user.GetExternalAPIUserForAccessTokenAndScope(token, scope, d.Store)
		...
		if err := user.SetExternalAPIUserAccessTokenAsUsed(token, d.Store); err != nil {
...
func RequireUserAccessToken(handler UserAccessTokenHandlerFunc, data *data.Service) http.HandlerFunc {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		user := user.GetUserByToken(accessToken, data.Store)
...
func RequireUserModerationScopeAccesstoken(handler http.HandlerFunc, data *data.Service) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		accessToken := r.URL.Query().Get("accessToken")
		if accessToken == "" {
			accessDenied(w)
