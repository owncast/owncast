...
// Defines values for PlaybackClientHealthMeasurementStatus.
const (
	Unmeasurable PlaybackClientHealthMeasurementStatus = "unmeasurable"
)

// Valid indicates whether the value is a known member of the PlaybackClientHealthMeasurementStatus enum.
func (e PlaybackClientHealthMeasurementStatus) Valid() bool {
	switch e {
	case Unmeasurable:
		return true
	default:
...
// Defines values for PlaybackClientHealthPlayerState.
const (
	A PlaybackClientHealthPlayerState = "a"
	E PlaybackClientHealthPlayerState = "e"
	K PlaybackClientHealthPlayerState = "k"
	P PlaybackClientHealthPlayerState = "p"
	W PlaybackClientHealthPlayerState = "w"
)

// Valid indicates whether the value is a known member of the PlaybackClientHealthPlayerState enum.
func (e PlaybackClientHealthPlayerState) Valid() bool {
	switch e {
	case A:
		return true
	case E:
...
// Defines values for PlaybackClientHealthSource.
const (
	Client PlaybackClientHealthSource = "client"
	Server PlaybackClientHealthSource = "server"
)

// Valid indicates whether the value is a known member of the PlaybackClientHealthSource enum.
func (e PlaybackClientHealthSource) Valid() bool {
	switch e {
	case Client:
		return true
	case Server:
...
// AdminConfigValueValue0 defines model for AdminConfigValue.Value.0.
type AdminConfigValueValue0 = string

// AdminConfigValueValue1 defines model for AdminConfigValue.Value.1.
type AdminConfigValueValue1 = bool

// AdminConfigValueValue2 defines model for AdminConfigValue.Value.2.
type AdminConfigValueValue2 = []string

// AdminConfigValueValue3 defines model for AdminConfigValue.Value.3.
...
// CmcdReportBody1 defines model for CmcdReportBody.1.
...
// PlaybackClient A single client currently playing back video, with its most recent playback health.
type PlaybackClient struct {
	// ClientID Playback identity of the client. The player-supplied CMCD session ID when it provides one, otherwise the same request-derived identity used for viewers.
	ClientID  string      `json:"clientID"`
	FirstSeen *time.Time  `json:"firstSeen,omitempty"`
	Geo       *GeoDetails `json:"geo,omitempty"`
	IpAddress *string     `json:"ipAddress,omitempty"`

	// Playback Null when no playback measurements exist for this client. A non-null playback object with measurementStatus explains why server-side measurements are unavailable.
	Playback  *PlaybackClientHealth `json:"playback,omitempty"`
	UserAgent *string               `json:"userAgent,omitempty"`

	// ViewerID Request-derived identity of the viewer this client belongs to. Clients sharing an address and user agent share a viewer identity.
	ViewerID string `json:"viewerID"`
}

// PlaybackClientHealth The most recent playback measurements for one client. A measurement is null when it is
// unknown for this client rather than measured as zero, because not every player reports every
// value, server-side observation cannot measure latency at all, and a value the player has
// stopped reporting expires rather than standing in for the current one.
type PlaybackClientHealth struct {
	// BandwidthKbps How fast video is reaching the client, either the player's own throughput estimate or the rate at which the server served its most recent segment. Server-side measurement may be unavailable when the server cannot see a trustworthy transfer, so clients report null unless the player measures itself.
	BandwidthKbps *float64 `json:"bandwidthKbps,omitempty"`

	// BitrateKbps Encoded bitrate of the quality variant the client is playing, as advertised in the stream playlist. That figure includes audio, so it is near but not equal to the configured video bitrate of the matching output variant. Only players that report it provide this.
	BitrateKbps *float64 `json:"bitrateKbps,omitempty"`

	// DownloadSeconds How long the client's most recent video segment took to transfer.
	DownloadSeconds *float64 `json:"downloadSeconds,omitempty"`

	// ErrorCount Errors, including buffering, since this client started playing. Null for a client the server can only observe, which reveals nothing about the player's errors.
	ErrorCount *float64 `json:"errorCount,omitempty"`

	// LastUpdate When the most recent measurement of any value for this client arrived.
	LastUpdate time.Time `json:"lastUpdate"`

	// LatencySeconds How far behind the live edge the client is playing. Only players that report it provide this, currently those supporting CMCD and those using the deprecated playback metrics endpoint.
	LatencySeconds *float64 `json:"latencySeconds,omitempty"`

	// MeasurementStatus Explains why the server could not produce a trustworthy segment measurement.
	MeasurementStatus *PlaybackClientHealthMeasurementStatus `json:"measurementStatus,omitempty"`

	// PlayerState Latest player state from CMCD, where p is playing, a is paused, w is buffering, k is seeking, and e is ended.
	PlayerState *PlaybackClientHealthPlayerState `json:"playerState,omitempty"`

	// QualityVariantChanges Quality variant switches since this client started playing. Null when the client reveals nothing about the quality it is playing.
	QualityVariantChanges *float64 `json:"qualityVariantChanges,omitempty"`

	// Source Whether the player is currently reporting its own playback, or the server is measuring it while serving segments. Individual values can still come from the other side, most often the segment download duration of a player that reports everything except how long its downloads took.
	Source PlaybackClientHealthSource `json:"source"`
}

// PlaybackClientHealthMeasurementStatus Explains why the server could not produce a trustworthy segment measurement.
type PlaybackClientHealthMeasurementStatus string

// PlaybackClientHealthPlayerState Latest player state from CMCD, where p is playing, a is paused, w is buffering, k is seeking, and e is ended.
type PlaybackClientHealthPlayerState string

// PlaybackClientHealthSource Whether the player is currently reporting its own playback, or the server is measuring it while serving segments. Individual values can still come from the other side, most often the segment download duration of a player that reports everything except how long its downloads took.
type PlaybackClientHealthSource string

// PlaybackMetrics defines model for PlaybackMetrics.
type PlaybackMetrics struct {
	// Bandwidth The player's measured throughput in kbps.
	Bandwidth *float64 `json:"bandwidth,omitempty"`

	// DownloadDuration How long the player's most recent segment took to download, in seconds.
...
// SetAutoplayJSONRequestBody defines body for SetAutoplay for application/json ContentType.
type SetAutoplayJSONRequestBody = AdminConfigValue
