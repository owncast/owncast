...
func RegisterFediverseOTPRequest(u user.User, w http.ResponseWriter, r *http.Request) {
	...
	msg := fmt.Sprintf("<If>This is an automated message from %s. If you did not request this message please ignore or block. Your requested one-time code is:</p><p>%s</p>", data.GetServerName(), reg.Code)
