...
// mockConfigRepository is a mock implementation for testing
type mockConfigRepository struct {
	federationEnabled   bool
	federationIsPrivate bool
	serverURL           string
	serverName          string
...
func (m *mockConfigRepository) GetFederationEnabled() bool {
	return m.federationEnabled
}

func (m *mockConfigRepository) GetFederationIsPrivate() bool {
	return m.federationIsPrivate
}
...
func (m *mockConfigRepository) GetDefaultFederationUsername() string {
...
func (m *mockConfigRepository) GetNSFW() bool {
	return m.nsfw
}

func (m *mockConfigRepository) GetFederationGoLiveMessage() string {
	return m.goLiveMessage
}
...
func contains(str, substr string) bool {
	return len(str) >= len(substr) && (str == substr || len(str) > 0 && len(substr) == 0 || (len(substr) > 0 && str != "" && str[0:len(str)] != "" && (str == substr || (len(str) > len(substr) && str[:len(substr)] == substr) || (len(str) > len(substr) && str[len(str)-len(substr):] == substr) || (len(str) > len(substr) && findSubstring(str, substr)))))
}
