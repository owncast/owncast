...
func TestHandleCreateWithNilId(t *testing.T) {
	...
	// Don't set JSONLD ID - should return nil since there is no object/note

	err := handleCreateRequest(context.Background(), activity)
	if err != nil {
		t.Errorf("handleCreateRequest with nil object should return nil (skip), got %v", err)
	}
}
...
func TestHandleCreateWithIdButNilIRI(t *testing.T) {
	...
	// Set the ID property but don't set an IRI on it - should return nil since there is no object/note
	activity.SetJSONLDId(id)

	err := handleCreateRequest(context.Background(), activity)
	if err != nil {
		t.Errorf("handleCreateRequest with no object should return nil (skip), got %v", err)
	}
}
