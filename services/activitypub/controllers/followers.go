...
func (c *Controllers) FollowersHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.URL.Query().Get("page") != "" {
		response, err = c.getFollowersPage(r.URL.Query().Get("page"), r)
	} else {
		response, err = c.getInitialFollowersRequest(r)
	}
...
func (c *Controllers) getInitialFollowersRequest(r *http.Request) (vocab.ActivityStreamsOrderedCollection, error) {
...
func (c *Controllers) getFollowersPage(page string, r *http.Request) (vocab.ActivityStreamsOrderedCollectionPage, error) {
	pageInt, err := strconv.Atoi(page)
	if err != nil {
		return nil, errors.Wrap(err, "unable to parse page number")
	}

	followerCount, err := c.followers.GetCount()
	if err != nil {
		return nil, errors.Wrap(err, "unable to get follower count")
	}

	followers, _, err := c.followers.GetFollowers(followersPageSize, (pageInt-1)*followersPageSize)
	if err != nil {
