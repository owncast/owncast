...
// the early-error path inside Add (nil IRI extraction), which returns
// before deps are touched.
var testOutbox = &Service{}
...
func TestAddWithNilId(t *testing.T) {
	...
	err := testOutbox.Add(note, "test-id", false)
	if err == nil {
...
func TestAddNoPanic(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			_ = testOutbox.Add(tc.item(), "test-id", false)
		})
	}
}
