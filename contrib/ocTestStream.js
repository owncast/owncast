...
class OcTestStream {
	...
	async getFFmpegPath() {
		return new Promise((resolve) => {
			const command =
				os.platform() === 'win32'
					? `where "${this.ffmpegExec}"`
					: `which "${this.ffmpegExec}"`;
	...
	showHelp() {
		console.log(
			'ocTestStream is used for sending pre-recorded or internal test content to an RTMP server.',
		);
		console.log('Usage: node ocTestStream.js [VIDEO_FILES] [RTMP_DESTINATION]');
		console.log(
			'VIDEO_FILES: path to one or multiple videos for sending to the RTMP server (optional)',
		);
		console.log(
			'RTMP_DESTINATION: URL of RTMP server with key (optional; default: rtmp://127.0.0.1/live/abc123)',
		);
	}
	...
	streamTestPattern() {
		console.log(
			`Streaming internal test video loop to ${this.destinationHost}`,
		);
		...
		const args = [
			...
			'lavfi',
			'-i',
			'testsrc=size=1280x720:rate=60[out0];sine=frequency=400:sample_rate=48000[out1]',
			'-vf',
			`[in]drawtext=fontsize=96:box=1:boxcolor=black@0.75:boxborderw=5${fontParam}:fontcolor=white:x=(w-text_w)/2:y=((h-text_h)/2)+((h-text_h)/-2):text='Owncast Test Stream',drawtext=fontsize=96:box=1:boxcolor=black@0.75:boxborderw=5${fontParam}:fontcolor=white:x=(w-text_w)/2:y=((h-text_h)/2)+((h-text_h)/2):text='%{gmtime\\:%H\\\\\\:%M\\\\\\:%S} UTC'[out]`,
			'-nal-hrd',
			'cbr',
			'-metadata:s:v',
			'encoder=test',
	 ...
	 */
	streamWithArgs(args) {
		const ffmpeg = spawn(this.ffmpegExec, args, {
			detached: false,
			stdio: 'inherit',
		});
	...
	streamVideoFiles() {
		...
		console.log(
			`Streaming a loop of ${this.videoFiles.length} video(s) to ${this.destinationHost}`,
		);
		if (this.videoFiles.length > 1) {
			console.log(
				'Warning: If these files differ greatly in formats, transitioning from one to another may not always work correctly.',
			);
		}
		...
		const args = [
			...
			'copy',
			'-vf',
			`drawtext=fontsize=96:box=1:boxcolor=black@0.75:boxborderw=5${fontParam}:fontcolor=white:x=(w-text_w)/2:y=((h-text_h)/2)+((h-text_h)/4):text='%{gmtime\\:%H\\\\\\:%M\\\\\\:%S}'`,
			'-f',
			'flv',
			this.destinationHost,
		];
	...
	async run() {
		...
		if (!(await this.findFFmpeg())) {
			console.error(
				'ERROR: ffmpeg was not found in path or in the current directory! Please install ffmpeg before using this script.',
			);
			process.exit(1);
		}
