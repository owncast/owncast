...
export const TabsAndChat: FC<TabsAndChatProps> = ({
  name,
  summary,
  tags,
  socialHandles,
  ...
  const items = [];
  if (showChat && currentUser && isMobile) {
    items.push({
      label: 'Chat',
      key: '0',
      children: (
  ...
  return (
    ...
    >
      {!isMobile && (
        <div
          id="skip-to-content"
          className={[styles.contentHeaderDesktop, showChat && styles.desktopPaddingForChat].join(
            ' ',
          )}
        >
          <ContentHeader
      ...
      <div className={[styles.tabSection, showChat && styles.desktopPaddingForChat].join(' ')}>
        {items.length > 1 && <Tabs defaultActiveKey="0" items={items} />}
      </div>
      <div className={[styles.noTabs, showChat && styles.desktopPaddingForChat].join(' ')}>
