...
func (ds *Datastore) warmCache() {
	...
	res, err := ds.DB.Query("SELECT key, value FROM Store")
...
func (ds *Datastore) Get(key string) (ConfigEntry, error) {
	...
	row := ds.DB.QueryRow("SELECT key, value FROM Store WHERE key = ? LIMIT 1", key)
...
func (ds *Datastore) Save(e ConfigEntry) error {
	...
	var stmt *sql.Stmt
	stmt, err = tx.Prepare("INSERT INTO Store (key, value) VALUES(?, ?) ON CONFLICT(key) DO UPDATE SET value=excluded.value")
	if err != nil {
		return err
	}
...
func (s *Service) Setup() {
	s.Store.cache = make(map[string][]byte)
	s.Store.DB = s.GetDatabase()

	createTableSQL := `CREATE TABLE IF NOT EXISTS Store (
		"key" string NOT NULL PRIMARY KEY,
		"value" BLOB,
		"timestamp" DATE DEFAULT CURRENT_TIMESTAMP NOT NULL
	);`

	s.Store.MustExec(createTableSQL)
	...
	if !s.hasPopulatedFederationDefaults() {
		...
		if err := s.Store.SetBool("HAS_POPULATED_FEDERATION_DEFAULTS", true); err != nil {
			log.Errorln(err)
		}
	}
